        ...
        with open(file_path, "r") as file:
            ...
            if variable:
                version_pattern = re.compile(
                    r'^\s*{}\s*=\s*["\'](?:beta-)?(\d{{4}}-\d{{2}}-\d{{2}})(?:-(\d{{3}}))?["\']\s*$'.format(re.escape(variable)),
                    re.MULTILINE
                )
            elif pattern:
                version_pattern = re.compile(pattern, re.MULTILINE)
            else:
            ...
            if version_match:
                ...
                if last_date == current_date:
                    try:
                        ...
                        print(f"Warning: Invalid build count '{last_count}' in {file_path}. Resetting to 1.")
                        build_count = 1
                else:
                    build_count = 1
            else:
                print(f"Version in {file_path} does not match expected format. Starting new versioning.")
...
def update_version_in_files(new_version, file_configs):
    ...
    for file_config in file_configs:
        ...
        if variable:
            version_pattern = re.compile(
                r'(^\s*{}\s*=\s*["\'])(.*?)(["\']\s*$)'.format(re.escape(variable)),
                re.MULTILINE
            )
        elif pattern:
            version_pattern = re.compile(pattern, re.MULTILINE)
